...
def trainword(keyword, window_radius = 3, learning_rate = 0.1, n_epochs = 10,batch_size = 1,nkerns = 1,filter_height=3,filter_width = 50, pool_height=1,pool_width = 1, loginput_num = 50, vector_size = 50, sequence = 0):
    ...
    input_size = nkerns*(2*window_radius)
    layer1 = HiddenLayer(
        rng,
        input=layer1_input,
    ...
    print len(layer0[0])
    for i in range(0, len(layer0[0])):
        params += layer0[0][i].params
        params += layer0[1][i].params
...
if __name__ == '__main__':
    ...
    output = open('result//traindouble'+str(window_radius)+str(vector_size)+str(nkerns)+str(loginput_num)+str(learning_rate)+args.keyword.decode('utf-8')+'.txt', 'wb')
