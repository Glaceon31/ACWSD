...
def trainword(keyword, window_radius = 3, learning_rate = 0.1, n_epochs = 10,batch_size = 1,nkerns = 1,filter_height=3,filter_width = 50, pool_height=1,pool_width = 1, loginput_num = 50, vector_size = 50, sequence = 0):
    ...
    layer0_input.append(x[0][0:vector_size*window_radius].reshape((batch_size, 1, window_radius, vector_size)))
    layer0_input.append(x[0][vector_size*(window_radius+1):vector_size*(2*window_radius+1)].reshape((batch_size, 1, window_radius, vector_size)))
    layer0_input.append(x.reshape((batch_size, 1, 2*window_radius+1, vector_size)))
    #layer0_input[1] = 
    layer0 = [[],[],[]]
    for i in range(1,window_radius+1):
        ph = window_radius-i+1
        print ph
        layer0[0].append(WsdConvPoolLayer(
            rng,
        input=layer0_input[0],
        image_shape=(batch_size, 1, window_radius, vector_size),
        filter_shape=(nkerns, 1, i, filter_width),
        poolsize=(ph, pool_width)
        ))
        layer0[1].append(WsdConvPoolLayer(
            rng,
        input=layer0_input[1],
        image_shape=(batch_size, 1, window_radius, vector_size),
        filter_shape=(nkerns, 1, i, filter_width),
        poolsize=(ph, pool_width)
        ))
    for i in range(1,2*window_radius+2):
        layer0[2].append(WsdConvPoolLayer(
            rng,
        input=layer0_input[2],
    ...
    layer0_output = []
    for i in range(0, len(layer0[0])):
        layer0_output.append(layer0[0][i].output)
    for i in range(0, len(layer0[1])):
        layer0_output.append(layer0[1][i].output)
    for i in range(0, len(layer0[2])):
        layer0_output.append(layer0[2][i].output)
    layer0_output = theano.tensor.concatenate(layer0_output)
    ...
    input_size = nkerns*(2*window_radius+2*window_radius+1)
    layer1 = HiddenLayer(
        rng,
        input=layer1_input,
    ...
    output_test2 = theano.function(
        [index],
        [layer0[0][0].output,layer1_input],
        #[layer1_input],
        givens={
            x: train_set_x[index * batch_size: (index + 1) * batch_size]
        }
    )
    #print trainsentence[1]
    #print output_test2(1)
    ...
    print len(layer0[0])
    for i in range(0, len(layer0[0])):
        params += layer0[0][i].params + layer0[1][i].params
    for i in range(0, len(layer0[2])):
        params += layer0[2][i].params
...
if __name__ == '__main__':
    ...
    output = open('result//traindoublemulti'+str(window_radius)+str(vector_size)+str(nkerns)+str(loginput_num)+str(learning_rate)+args.keyword.decode('utf-8')+'.txt', 'wb')
