...
def crf(keyword):
    ...
    for i in range(0, len(testsentence)):
        for j in range(0, len(testsentence[i])/2):
            if testsentence[i][j] == ' ':
                continue
            testcontent += testsentence[i][j]+' '+'X\n'
        testcontent += testsentence[i][len(testsentence[i])/2]+' '+senselist[test_set_y[i].eval()]+'\n'
    ...
    time.sleep(1)
    os.system('crf_test -m crf/model/'+keyword.encode('utf-8')+'_crf crf/test/'+keyword.encode('utf-8')+'_crf.txt')
